...
const submitStart = js.indexOf('async function submitOVN(e) {');
if (submitStart === -1) throw new Error("submitOVN not found");
const submitEnd = js.indexOf('};', submitStart) + 2;
// Check if the original also had `window.submitOVN = submitOVN;` right after it
js = js.substring(0, submitStart) + submitLogic.trim() + js.substring(submitEnd);

let cleanupWindowAssign = js.indexOf('window.submitOVN = submitOVN;');
if (cleanupWindowAssign !== -1) {
    js = js.replace('window.submitOVN = submitOVN;', '');
}
